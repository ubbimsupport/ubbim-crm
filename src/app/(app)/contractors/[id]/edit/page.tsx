...
export default async function EditContractorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!company || company.company_kind !== "contractor") notFound();
  ...
  return (
    <div>
      ...
      <CompanyForm kind="contractor" company={company} categories={(categories ?? []) as Category[]} staff={(staff ?? []) as Profile[]} />
    </div>
  );
}
